...
async function main() {
  const c = await hre.ethers.deployContract("ReserveAttestationV2", [QUORUM, MAXAGE, MININT]);
  await c.waitForDeployment();
  const a = await c.getAddress();
  console.log("ReserveAttestationV2:", a, `(quorum ${QUORUM}, maxAge ${MAXAGE}s, minInterval ${MININT}s)`);
